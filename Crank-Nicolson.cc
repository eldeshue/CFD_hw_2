/* 2022.10.14
 * 전산 유체 역학 과제 2
 * solving couette flow, Crank-Nicolson
...
int main(){
  ...
  // set Gnuplot object;
	...
	double h = 0.04;
	int JM = 41;
	int NM = 109; // 1) 541    2) 109
	double delta_y = 0.001; 
	double delta_t = 0.01; // 1) 0.002, 2) 0.01
  ...
  std::vector<std::vector<std::pair<double,double>>> data(NM,std::vector<std::pair<double,double>>(JM));
	...
	for(int n = 1; n < NM; n++){
		...
		diffusion_factor * (data[n - 1][MAX_Y].first - 2 * data[n - 1][MAX_Y - 1].first + data[n - 1][MAX_Y - 2].first 
											+ data[n][MAX_Y].first) + 2 * data[n - 1][MAX_Y - 1].first;
	...
	gp << "set title 'Velofity profiles obtained by Crank-Nicolson' font 'Helvetica, 13'" << std::endl;
